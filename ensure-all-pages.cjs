  ...
  for (const page of missingPages) {
    if (!json[page.slug]) {
      json[page.slug] = {
        ...
        keywords: page.title.split(' ')[0] + ' calculator',
        schema: '',
        body: '<section class="card"><h2>' + page.h1Title + '</h2><p>' + page.description + '</p><p>To calculate any value, use the <a href="/amps-to-watts-calculator/">Amps to Watts Calculator</a>.</p></section>'
      };
    }
  }

  fs.writeFileSync(legacyPath, JSON.stringify(json, null, 2));
  console.log('Updated legacy-pages.json with all missing pages.');
} catch (e) {
  console.error(e);
}
...
try {
  ...
  if (newPagesString) {
    toolsContent = toolsContent.replace(/];\s*$/, newPagesString + '];\n');
