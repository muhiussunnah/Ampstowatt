...
# Directory to scan
WORKSPACE_DIR = r"d:\Git\ampstowatts\Ampstowatt"
...
def build_site():
    ...
    css_links_regex = re.compile(r'<link\s+rel="stylesheet"\s+href="/luxury-tools\.css.*?>\s*<link\s+rel="stylesheet"\s+href="/styles\.min\.css.*?>\s*<link\s+rel="stylesheet"\s+href="/site-dark-luxury\.css.*?>', re.DOTALL | re.IGNORECASE)
    
    optimized_css = '''  <link rel="preload" href="/site-dark-luxury.min.css?v=build" as="style">
  <link rel="stylesheet" href="/site-dark-luxury.min.css?v=build">
  <link rel="stylesheet" href="/luxury-tools.min.css?v=build" media="print" onload="this.media='all'">
  <link rel="stylesheet" href="/styles.min.css?v=build" media="print" onload="this.media='all'">'''
    ...
    for root, dirs, files in os.walk(WORKSPACE_DIR):
        ...
        for filename in files:
            if filename.endswith('.html'):
                ...
                content = re.sub(r'\s*onkeydown="[^"]+"', '', content)

                # 5. Optimize CSS Delivery (Fix Render Blocking)
                content = css_links_regex.sub(optimized_css, content)
