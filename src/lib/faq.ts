...
export function getFaqQuestions(tool?: Tool): FaqItem[] {
  if (tool) {
    ...
    if (tool.kind === 'power-factor' || tool.kind === 'kva-to-watts' || tool.kind === 'volt-amps') return powerQualityFaq(tool);
    if (tool.kind === 'amps-to-kw') return ampsToKwFaq();
    if (tool.slug.includes('megawatts')) return megawattFaq();
    if (tool.defaultPhase === 'ac3' || tool.slug.includes('3-phase')) return threePhaseFaq();
    if (tool.defaultPhase === 'dc' || tool.slug.includes('dc') || tool.slug.includes('12v')) return dcFaq(tool);
