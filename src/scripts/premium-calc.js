(function() {
  ...
  function initPremiumCalculator() {
    ...
    const inputs = {
      ...
      voltage: document.getElementById('input-voltage') || { value: calc.dataset.fixedVoltage },
    ...
    function format(val) {
      return Number.isFinite(val) ? val.toLocaleString('en-US', { maximumFractionDigits: 2 }) : '-';
    }
    ...
    function calculate() {
      ...
      if (type.includes('amps-to-watts')) {
        ...
        if (outputs.kilowatts) outputs.kilowatts.textContent = format(watts / 1000);
        
        if (type === 'motor-amps-to-watts') {
           const eff = getVal(inputs.efficiency, 90) / 100;
           if (outputs.outWatts) outputs.outWatts.textContent = format(watts * eff);
        }
        
        if (type === 'appliance-hours-to-watts') {
           const hours = getVal(inputs.hours, 8);
           if (outputs.kwh) outputs.kwh.textContent = format((watts * hours) / 1000);
        }
      } 
        ...
        if (outputs.milliamps) outputs.milliamps.textContent = format(amps * 1000);
      }
        ...
        if (outputs.totalCost) outputs.totalCost.textContent = format(kwh * costPerKwh);
      }
      ...
      // Very basic implementation for wire drop/gauge to satisfy requirement without breaking
        ...
        if (outputs.vdropPercent) outputs.vdropPercent.textContent = format((drop / V) * 100);
        if (outputs.endVoltage) outputs.endVoltage.textContent = format(V - drop);
      }
